...
const newQuestionsPath = path.join(process.cwd(), 'src/lib/polish_batch.json');

const questionsContent = fs.readFileSync(questionsPath, 'utf8');
const newQuestions = JSON.parse(fs.readFileSync(newQuestionsPath, 'utf8'));

// Robust find for insertion point
const variableDecl = 'export const SAMPLE_QUESTIONS: Question[] = [';
let startIdx = questionsContent.indexOf(variableDecl);
...
console.log(`Successfully ingested ${newQuestions.length} questions from Polish Batch.`);
